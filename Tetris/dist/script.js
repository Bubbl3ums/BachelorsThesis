...
Page.Game = new DrawAreaObj(0,0,10,20,function(){
  ...
  var Now = new Date()
  if (!GM.IsAlive && (Now - StartTime > 300000)){
    sendData("control");         
    DrawText("RETURN TO SURVEY",'rgb(255,255,255)','500',
             'center',uDrawSize,this.W/2,this.H/4);
  }
  else if (!GM.IsAlive && (Now - StartTime < 300000)){
    DrawText("PRESS ANY BUTTON",'rgb(255,255,255)','500',
             'center',uDrawSize,this.W/2,this.H/4);
    DrawText("TO START",'rgb(255,255,255)','500',
             'center',uDrawSize,this.W/2,this.H/3);
  }
});
...
var GM = {
  ...
  Update: function(){
    ...
    //this limits the player to a playtime of 5 minutes
    var Now = new Date()
  ...
  GameOver: function(){
    ...
    if (this.ScoreCur > this.ScoreHigh){
      ...
      localStorage.setItem("HighScore", this.ScoreHigh)
    }    
    
    Deaths ++
    localStorage.setItem("Deaths", Deaths)
    console.log("Total Deaths is now:", Deaths)
