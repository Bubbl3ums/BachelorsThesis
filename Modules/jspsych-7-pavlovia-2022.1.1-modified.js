	...
	class PavloviaPlugin
	{
		...
		async trial(display_element, trial)
		{
			...
			switch (trial.command.toLowerCase())
			{
				...
				case "finish":
					this._jsPsych.data.get().trials = this._jsPsych.data.get().trials.filter(item => item.trial_type !== 'pavlovia')
					const data = this._jsPsych.data.get().csv();
					console.log("FINISHING DATA: ", data)
		 ...
		 *
		 * @param {Object} trial - the jsPsych trial
		 * @param {string} [configURL= "/gaianaumann/thesis-action-game/config.json"] - the URL of the pavlovia.org json configuration file
		 * @returns {Promise<void>}
		 * @private
		 */
		async _init(trial, configURL = '/gaianaumann/thesis-action-game/config.json')
		{
			try
			{
				// configure:
		 ...
		 * of the experiment.
		 *
		 * @param {string} [configURL= "/gaianaumann/thesis-action-game/config.json"] - the URL of the pavlovia.org json
		 * 	configuration file
		 * @returns {Promise<any>}
		 * @private
		 */
		...
		async _save(trial, data, sync = false)
		{
			...
			const key = trial.participantId + '_' + dateString + '_' + trial.gameType + '.csv';
