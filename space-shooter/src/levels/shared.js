...
var terminateLevel = function(level, handler, bossExplosionAnimTime){
	...
	//For printing the time spent since the beginning at the end of every level
	const dEnd = new Date();
	let endtime = dEnd.getTime();
	
	//calculates the total time spent on the game and saves it in local storage 
	localStorage.setItem("totalTime", endtime - Number(localStorage.getItem("startTime")))
	console.log("Total Time:", localStorage.getItem("totalTime"))
	...
	handler.delay(function(){
		...
		//stores total death counter in local storage
		localStorage.setItem ("totalDeaths", Player.deathCount)
		console.log("Total Deaths:", localStorage.getItem("totalDeaths"))

		if (level == 2){
			sendData("experimental"); 
		};
